...
alternatives
    ...
    tklps (TurnKey GNU/Linux Packaging System)
...
<t tx="zaril.20101101154405">logic
    ...
    GOTCHA: programs running under seteuid don't get the dropped privileges
        they get the full privileges
            we could implement our own system

    IDEA: dedicated code for dropping privileges transparently in a sub-process

    
IDEAS
    drop privileges via decorator
    stress test pool as root



E: have the pool internal directories setuid to the user who initialized it? If root executes the pool, the pool should create a file inside the internals it should be owned by 
R: doesn't work

E: have the pool temporarily drop privileges to the user who owns the pool
R: 


Q: which files are owned by root after pool-get?
A:
find .pool -user root

./build/logs/pyproject_0+2010.11.12+05.32.34+58133553.build
./pkgcache/pyproject_0+2010.11.12+05.32.34+58133553_i386.deb
./stocks/pyproject#master/checkout/.git/HEAD
./stocks/pyproject#master/checkout/.git/index
...
IDEAS
    ...
    detect a Sumo type Stock and create separate class for handling Sumo stocks?

where do we have Sumlo specific code?

Stock._get_workdir
    if its a sumo arena then
        then thin branch is also dupped
        sumo arena is opened

    IDEA: we could do the sumo check in an overriden _get_workdir

Stock.unregister
    if its a sumo arena then we close it
        we can't implement that by overriding the method

    unless we implemented a special cleanup hook
...
Q: how do we store it?

Q: are source versions for plain identified?
A: yes they are
...
when we import a package into the cache, set the filename to includes epoch appropriately
    cache package information so we don't have to re-extract it?
        optimize later - test it works first! (and then test performance first)
            md5sum should be much much faster
...
Q: where are packages imported?
A: pkgcache.add

    
Q: why don't package's have version epochs in the filename?
    E: try renaming a package to include epoch

    dpkg -f package.deb 
        shows control fields
    dpkg -I package.deb
        shows info/summary
...
<t tx="zaril.20101120203018">SOLUTION: 
    ...
    preserve suffix when we are importing the pkg into the pkgcache

Q: what are udebs?
A: stripped down regular packages for debian-instaler

the u means 'micro'
they have exactly the same structure as regular debs
...
<t tx="zaril.20101121151810">SUMMARY:
    ...
    when we can cache the debinfo key, looking up the control file in a cache is 120X faster than extracting it every time by a subcommand

    if we can't cache the debinfo key, then we need to calculate  

    bottom line: 60-80X improvement 
        before: 70-90 packages a sec
        after: 4000-7250 packages a sec

* RESULTS PHASE #1

optimization: cache control file by md5 of Debian package

test setup
    dumped rocky packages to a plain directory
    created p13 registered that plain directory
    
pool-list
    reminder - has to be rescanned on every invocation
...
Q: is caching the control file effective?
A: the bottleneck is in IO for the digest. 
For small files it is much much more efficient:
    bench(lambda: get_control_by_path(package), 500)
    500 runs in 0.1935 seconds (2584.15 per/sec)
    
    bench(lambda: get_control_by_path(package, usecache=False), 500)
    500 runs in 4.9849 seconds (100.30 per/sec)
    
* RESULTS PHASE #2

optimizations
    pure Python extraction of control file
    calculate cache key by hashing just the control.tar.gz
...
AFTER 
(cached) 10X improvement
real    0m0.335s
user    0m0.232s
sys     0m0.076s

(not cached - I.e., rm -rf ~/.debinfo): 5X improvement
real    0m2.513s
user    0m2.156s
sys     0m0.224s

E: test unbuffered performance
R:
    Q: how do I clear the disk buffers?
    A: read more than 1gb?
    
    dd if=/dev/zero dd if=/dev/zero of=zero bs=1M count=1024
    dd if=zero of=zero2 bs=1M count=1024
...
Q: do we allow generic key listing?
A: we could, but only hashes
...
<t tx="zaril.20101121182502">   ncalls  tottime  percall  cumtime  percall filename:lineno(function)
        ...
        1    0.000    0.000    5.350    5.350 pool.py:675(list)
        1    0.000    0.000    5.310    5.310 pool.py:479(sync)
        1    0.000    0.000    5.310    5.310 pool.py:842(sync)
        1    0.000    0.000    5.310    5.310 pool.py:536(wrapper)
        1    0.000    0.000    5.310    5.310 pool.py:335(sync)
        1    0.030    0.030    5.310    5.310 pool.py:317(_sync)
      595    0.230    0.000    5.090    0.009 debinfo.py:54(extract_control_fields)
      595    0.000    0.000    5.090    0.009 pool.py:110(add)
      595    0.020    0.000    4.360    0.007 debinfo.py:44(extract_control)
      595    0.660    0.001    4.150    0.007 debinfo.py:22(hashfile)
    33456    2.890    0.000    2.890    0.000 :0(update)
    34646    0.620    0.000    0.620    0.000 :0(read)
     6957    0.100    0.000    0.330    0.000 sre.py:154(split)
      595    0.030    0.000    0.190    0.000 hashstore.py:20(get)
      2/1    0.000    0.000    0.170    0.170 pool.py:610(__init__)
        1    0.010    0.010    0.150    0.150 pool.py:73(__init__)
     6976    0.100    0.000    0.140    0.000 sre.py:213(_compile)
...
but we need to extract it from Python
    ...
    maybe there is python code to do this?

linux-image-2.6.20-15
...
<t tx="zaril.20101121210322">SUMMARY
    ...
    optimized version: 12K per/sec
        54X faster than executing ar
...
48 	57 	File size in bytes 	Decimal
    ar files limited to 953 MBs!
58 	59 	File magic 	\140\012   `\n

test
    setup archive with three files
        test ability to extract any of them



* SCRATCH

ar x file.deb control.tar.gz
    extracts file
ar p file.deb control.tar.gz
    prints file

ar t file.deb
    shows file in archive

ar d file.deb control.tar.gz
    delete control file deb

ar r file.deb control.tar.gz
    add control.tar.gz to package

ar rc archive file 
    create archive with file in it

ar rc fromscratch.deb debian-binary control.tar.gz data.tar.gz

nm --print-armap archive
    shows files in archive

E: create deb from scratch and try to install it?
R: it works!
---
E: try changing order of files
R: it works!


Q: how big is the entire header?
A: 0x8 0x44
    exactly 60 bytes
---
...
E: how much of an improvement if we turn on psyco?
    2X improvement for the small file
        similar improvement for the larger file
...
<t tx="zaril.20101125202956">SUMMARY
    ...
    putting the instance inside a subprocess eliminates most of this overhead
        operations become constant time
            previously the overhead seemed to grow exponentially with the amount of packages you ask for
    

    EXAMPLE BENCHMARKS
        BEFORE:    
            time pool-get tmp/ -i 50pkgs
                real    2m8.659s
                user    2m2.628s
                sys     0m2.036s
...
IDEAS
    maybe keep the entire object in reduced privileges?
        we would communicate with the object in RPC

    batch method invocation?

    the pool would drop privileges on initialization
        and run transparently in a separate process
        only the proxy would run with root privileges

drop privileges at the 
    
Q: how can we reduce how often drop_privileges is called?

Q: what dropped pool methods does cmd_get call?
...
design of RPC mechanism

communicate via pty
the subprocess reads requests

one method per socket connection 
    e.g., HTTP/1.0 style

writes back the return value

write size and then the serialized message
    we don't need to do that, since pickle will do the work for us

fh = sock.makefile("rw", 0)

closing the makefile fh not enough to close the connection
...
E: experiment with pickling things back and forth across a socket

write will eventually block
...
GOTCHA: ptys and pipes are NOT bi-directional
    the only difference between a pipe and a pty is that pipes have termios control options

Q: does fh.closing an fdopen'ed file close the pipe?
A: yes

* ROADMAP
    foo.add
        static structure in proxy and server
...
Q: can we serialize functions between processes?

configure Proxy object as inheritable
    
ROADMAP
    convert call to InstanceServer/InstanceClient
    get 
    set
...
benchmarks for forked_constructor
    runs 2K/sec with pickle
    runs 8.5K/sec with cPickle
    
    not proxied: 774K/sec (100X faster)

benchmarks for forked_func
...
<t tx="zaril.20101126233132">LOGIC
    ...
    wrap a dummy constructor which:
        drops privileges
        return the already created instance (this will be wrapped)

IDEAS
    it would be nice if we could pass an existing instance
        we can, just create a function that returns an existing instance

GOTCHA:
    I need attribute access for cmd_info.py
...
SOLUTION:
    drop privileges of the entire info command
        maybe also in cmd_info_build 

INSIGHT: dropping privileges should be the default
    it fits all commands except get

I think these are sync operations in info

print_pkgcache
print_stock_sources
pritn_stock_binaries

E: remove sync in cmd_info
R: that solves the problem, so its definitely there

IDEAS
    drop privileges in cmd_info?
    somehow intercept the pool after its come
...
* ROADMAP
pass everything to PoolKernel
privilege dropping 
    remove from cmd_get.py
move init_create to Pool
list
get
...
create option NOT to retain privileges
    so we can drop privileges globally
        useful in some of the cli commands
            don't need overhead of additional fork
                only relevant when there is a privilege gap (root/fab)
                    better to use API anyhow in that scenario

makerelative from Paths
...
IDEAS:
list accepts only one package_glob and raises an exception if its missing?
    very inefficient

returns a data structure inherited from list
    list of packages
